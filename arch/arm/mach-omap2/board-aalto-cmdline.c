/* Custom command line for Samsung YP-GS1
...
void __init aalto_cmdline_set_serialno(void)
{
	/*
...
static struct cmdline_parameter cmdline_parameters[] __initdata = {
        {"omapfb.vram", "0:1200K", 0},
        {"androidboot.hardware", "aalto", 1},
        {"mem", "512M", 1},
};
...
void __init aalto_manipulate_cmdline(char *default_command_line)
{
        char *s, *p, *p2;
        int i;
        int ws;
		...
		const char *tag_command_line, size_t size)
{
	...
	aalto_manipulate_cmdline(default_command_line);
}
#endif
