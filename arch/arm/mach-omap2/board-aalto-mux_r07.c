/*
 * linux/arch/arm/mach-omap2/board-AALTO-mux_r08.c
 *
...
#define AALTO_OMAP_OUTPUT_GPIO(name, val)		{name, val, (unsigned int)#name},

static unsigned int aalto_board_output_gpio[][3] = {
	AALTO_OMAP_OUTPUT_GPIO(OMAP_GPIO_PS_HOLD_PU, 0)
	AALTO_OMAP_OUTPUT_GPIO(OMAP_GPIO_FM_nRST, 0)
	AALTO_OMAP_OUTPUT_GPIO(OMAP_GPIO_CAM_CIF_NRST, 0)
	AALTO_OMAP_OUTPUT_GPIO(OMAP_GPIO_CAM_CIF_NSTBY, 0)
	AALTO_OMAP_OUTPUT_GPIO(OMAP_GPIO_CAM_5M_NSTBY, 0)
	AALTO_OMAP_OUTPUT_GPIO(OMAP_GPIO_AMP_SHDN, 1)
	AALTO_OMAP_OUTPUT_GPIO(OMAP_GPIO_JIG_ON18, 0) 
	AALTO_OMAP_OUTPUT_GPIO(OMAP_GPIO_VIBTONE_EN, 0)
	AALTO_OMAP_OUTPUT_GPIO(OMAP_GPIO_LCD_EN_SET, 0)
	AALTO_OMAP_OUTPUT_GPIO(OMAP_GPIO_LED_EN1, 0)
	AALTO_OMAP_OUTPUT_GPIO(OMAP_GPIO_LED_EN2, 0)
	AALTO_OMAP_OUTPUT_GPIO(OMAP_GPIO_CP_RST, 1)
	AALTO_OMAP_OUTPUT_GPIO(OMAP_GPIO_IPC_MRDY, 0)
	AALTO_OMAP_OUTPUT_GPIO(OMAP_GPIO_PDA_ACTIVE, 0)
	AALTO_OMAP_OUTPUT_GPIO(OMAP_GPIO_PCM_SEL, 0)
	AALTO_OMAP_OUTPUT_GPIO(OMAP_GPIO_MASSMEMORY_EN, 1)
	AALTO_OMAP_OUTPUT_GPIO(OMAP_GPIO_ALS_EN, 0)
	AALTO_OMAP_OUTPUT_GPIO(OMAP_GPIO_CON_CP_SEL, 0)
	AALTO_OMAP_OUTPUT_GPIO(OMAP_GPIO_UART_SEL, 1)
	AALTO_OMAP_OUTPUT_GPIO(OMAP_GPIO_EARPATH_SEL, 0)
	AALTO_OMAP_OUTPUT_GPIO(OMAP_GPIO_EN_TEMP_VDD, 0)
};	/* end array __omap_output_gpio */

static unsigned int aalto_board_wakeup_gpio[] = {
	OMAP_GPIO_JACK_NINT,
	OMAP_GPIO_IPC_SRDY,
	OMAP_GPIO_CHG_ING_N,
...
static struct omap_board_mux aalto_board_core_mux[] = {
...
int __init aalto_mux_init(void)
{
#ifdef CONFIG_OMAP_MUX
	return omap3_mux_init(aalto_board_core_mux, OMAP_PACKAGE_CBP);
#else
	return omap3_mux_init(NULL, OMAP_PACKAGE_CBP);
#endif
}

int __init aalto_mux_init_padconf(void)
{
	int ret = 0;
	unsigned int i;

	for (i = 0; i < ARRAY_SIZE(aalto_board_core_mux); i++) {
		ret = omap_cfg_reg(i);
		if (ret) {
...
int __init aalto_mux_init_gpio_out(void)
{
	int ret = 0;
	int err = 0;
	unsigned int i = 0;

	for (i = 0; i < ARRAY_SIZE(aalto_board_output_gpio); i++) {
		err = gpio_request(aalto_board_output_gpio[i][0],
				   (char *)aalto_board_output_gpio[i][2]);
		if (err < 0) {
		...
		gpio_direction_output(aalto_board_output_gpio[i][0],
				      aalto_board_output_gpio[i][1]);
	}
...
int __init aalto_mux_set_wakeup_gpio(void)
{
	int ret = 0;
	unsigned int i = 0;

	for (i = 0; i < ARRAY_SIZE(aalto_board_wakeup_gpio); i++)
		enable_irq_wake(OMAP_GPIO_IRQ(aalto_board_wakeup_gpio[i]));
