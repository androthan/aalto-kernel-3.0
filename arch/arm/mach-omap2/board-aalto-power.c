/* Power support for Samsung YP-GS1
...
/* Based on mach-omap2/board-latona-power.c
 */
...
#define HIGH_BLOCK_TEMP_AALTO		650
#define HIGH_RECOVER_TEMP_AALTO	550
#define LOW_BLOCK_TEMP_AALTO		(-30)
#define LOW_RECOVER_TEMP_AALTO		10
...
static char *aalto_charger_supplied_to[] = {
	"battery",
};

static int aalto_adjust_soc(int value)
{
	if(value == 100)
		value = 100;
	else if(value < 30)
...
static const __initdata struct pda_power_pdata charger_pdata = {
	...
	.supplied_to = aalto_charger_supplied_to,
	.num_supplicants = ARRAY_SIZE(aalto_charger_supplied_to),
...
static struct max17040_platform_data max17040_pdata = {
	...
	.adjust_soc = aalto_adjust_soc,
	.get_bat_temp = get_bat_temp_by_adc,
	.high_block_temp = HIGH_BLOCK_TEMP_AALTO,
	.high_recover_temp = HIGH_RECOVER_TEMP_AALTO,
	.low_block_temp = LOW_BLOCK_TEMP_AALTO,
	.low_recover_temp = LOW_RECOVER_TEMP_AALTO,
...
static const __initdata struct i2c_board_info aalto_i2c4_boardinfo[] = {
	{
...
void __init aalto_power_init(void)
{
	...
	i2c_register_board_info(4, aalto_i2c4_boardinfo, ARRAY_SIZE(aalto_i2c4_boardinfo));
