...
extern void start_lcd_esd_wdog(void);
extern int get_hw_revision(void);

static	u8	m_ts_debug_mode = ZINITIX_DEBUG;
//#define	SYSTEM_MAX_X_RESOLUTION	800 //4000	//480
//#define	SYSTEM_MAX_Y_RESOLUTION	480 //3900	//800
...
// test code for insert delay after AFE scan by taejin
#define ZINITIX_SCAN_DELAY	0xb7

#define GPIO_TOUCH_PIN_NUM  	TSP_INT	// interrupt pin number
#define GPIO_TOUCH_IRQ  	OMAP_GPIO_IRQ( TSP_INT )	// interrupt pin IRQ number
...
#endif
static struct zinitix_touch_dev * touch_dev_resume = NULL;
#if	ZINITIX_DEBUG
...
static struct regulator *vreg_touch1 = NULL;
static struct regulator *vreg_touch2 = NULL;
//-------------------------------------------------------
typedef	struct	
{
	u16	x;
...
#define ZINITIX_DRIVER_NAME        "zinitix"
...
static int check_binary = 0;
static int set_test_mode = 49;

static int need_hw_calibration = 0;
...
void ts_power_control(zinitix_touch_dev *touch_dev, u8 ctl)
{
	// taejin_sec
	int ret33 = 0;
	int ret18 = 0;	
	printk(KERN_INFO "TSP ts_power_control\n");
	if (get_hw_revision() <= 0x07) {
		if(vreg_touch1 == NULL)	{
			vreg_touch1 = regulator_get(NULL, "vmmc2");
			if(IS_ERR(vreg_touch1))
				printk("Fail to register vreg_touch1(vmmc2) in touch driver\n");
		}
		if(vreg_touch2 == NULL) {
			vreg_touch2 = regulator_get(NULL, "vdda_dac1");
			if(IS_ERR(vreg_touch2))
				printk("Fail to register vreg_touch2(vdac2) in touch driver\n");
		}

		if(ctl == POWER_OFF) //power off
		{
			printk("tsp_power off\n");
			ret33 = regulator_disable(vreg_touch1);
			ret18 = regulator_disable(vreg_touch2);
			if (ret33) {
		...
		{
			printk("tsp_power on\n");
			ret33 = regulator_set_voltage(vreg_touch1, 3150000, 3150000);
			if (ret33) {
				printk("[TSP] Error,%s: set voltage failed (%d)\n",
						__func__, ret33);
			}
			ret33 = regulator_enable(vreg_touch1);
			ret18 = regulator_enable(vreg_touch2);
	...
	} else { // using external ldo
		if (ctl == POWER_OFF) { //power off
			gpio_direction_output(OMAP_GPIO_AMP_SHDN, 0);
			gpio_free(OMAP_GPIO_AMP_SHDN);
		} else if (ctl == POWER_ON) {	//power on
			ret33 = gpio_request(OMAP_GPIO_AMP_SHDN, "TSP_LDO");
			if (ret33 < 0) {
				printk("fail to get gpio : %d, res : %d\n", OMAP_GPIO_AMP_SHDN, ret33);
			}
...
static void zinitix_touch_tmr_work(struct work_struct *work)
{
	...
	printk("error. timeout occured. maybe ts device dead. so reset & reinit.\r\n");
...
u8 ts_upgrade_firmware(zinitix_touch_dev* touch_dev, const u8 *firmware_data, u32 size)
{
	...
	ts_read_data(touch_dev->client, ZINITIX_FIRMWARE_VERSION, (u8*)&chip_firmware_version, 2);
	chip_firmware_version = chip_firmware_version & 0x00FF;
	printk("TSP frimware show : %02d\n", chip_firmware_version);
	...
	temp_data = (u8*)kzalloc(size, GFP_KERNEL);
	if(temp_data == NULL)
	{
		printk(KERN_ERR "cannot alloc temp buffer\n");
		return false;
	}
	...
	do{
...
#else
		printk("Erase Flash w/o HW_Calibration\n");
		...
		for(flash_addr= 0; flash_addr< size; )
		{
			...
			if(flash_addr >= CALIBRATION_AREA) {
				printk("TSP, pass calibration data area\n");
				break;
			}
#endif
		}
...
#if !USE_HW_CALIBRATION
			memcpy((u8*)&temp_data[0], (u8*)&verify_data[0], size);
			memcpy((u8*)&verify_data[0], (u8*)&temp_data[size-CALIBRATION_AREA], CALIBRATION_AREA);
		...
		{
#if USE_HW_CALIBRATION
			printk("TSP, need_hw_calibration is 1\n");
			need_hw_calibration = 1;
#endif
...
retry_init:
	...
	{
		printk("fail to write interrupt register\r\n");
		goto fail_init;
	}
	...
	{
		printk("fail to read chip revision\r\n");
		goto fail_init;
	}	
	printk("zinitix touch chip revision id = %x\r\n", chip_revision);
 	...
 	if(ts_check_need_upgrade(chip_firmware_version, chip_reg_data_version)==true)
   	{
		...
		/* get chip firmware version */
		if (ts_read_data(touch_dev->client, ZINITIX_FIRMWARE_VERSION, (u8*)&chip_firmware_version, 2)<0) goto fail_init;		
		printk("zinitix touch chip renewed firmware version = %x\r\n", chip_firmware_version);
	...
	if (need_hw_calibration) {
		printk("TSP, need calibration\n");
		if(zinitix_bit_test(chip_eeprom_info, 0))		// hw calibration bit
		{
			printk("TSP, do calibration\n");
			 // h/w calibration
			udelay(100);
			for(i=0;i<10;i++)
			{
			...
			for(i=0;i<10;i++)
			{
				...
				do{
					...
					if(hw_cal_cnt>20)
					{
						zinitix_debug_msg("could not calibrated\r\n");
						mdelay(100);
						break;
					}
				}while(1);
...
#endif
{
	...
	if(touch_dev->raw_mode_flag == TOUCH_TEST_RAW_MODE)	// test mode
	{
		if (ts_write_reg(touch_dev->client, ZINITIX_PERIODICAL_INTERRUPT_INTERVAL, ZINITIX_SCAN_RATE_HZ*ZINITIX_RAW_DATA_ESD_TIMER_INTERVAL)!=I2C_SUCCESS)
			printk("[zinitix_touch] Fail to set ZINITIX_RAW_DATA_ESD_TIMER_INTERVAL.\r\n");
	}
	else
#endif
{
...
#endif	// USE_THREAD_METHOD
{
	...
	for (;;)
	{
		...
		do
		{
			...
			if(touch_dev->work_proceedure != TS_NO_WORK)
			{
				zinitix_debug_msg("zinitix_touch_thread : [warning] other process occupied.work_proceedure is %d.\r\n",touch_dev->work_proceedure );
#if DELAY_FOR_SIGNAL_DELAY	
				udelay(DELAY_FOR_SIGNAL_DELAY);
#endif				
				if (!gpio_get_value(touch_dev->int_gpio_num))
...
// test code for insert delay after AFE scan by taejin
void zinitix_change_scan_delay(int value)
{ 
	zinitix_touch_dev * touch_dev;
	if (touch_dev_resume != NULL) {
		touch_dev = touch_dev_resume;
		ts_write_reg(touch_dev->client, ZINITIX_SCAN_DELAY, value);
	}
	return;        
}                      
EXPORT_SYMBOL(zinitix_change_scan_delay);
...
void zinitix_resume(void)
{
	...
	touch_dev = touch_dev_resume;
	printk("resume++\r\n");
	if(touch_dev_resume == NULL)	return;
...
static ssize_t zinitix_show_info(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	u16     chip_firmware_version=0;
	ts_read_data(touch_dev->client, ZINITIX_FIRMWARE_VERSION, (u8*)&chip_firmware_version, 2);
	printk("TSP frimware show : %02d\n", (chip_firmware_version & 0x00FF));
	count += sprintf(buf, "%02d\n", (chip_firmware_version & 0x00FF));
	return count;
}
/*
static ssize_t zinitix_update_fw(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk("zinitix update fw : name = %s, size = %d\r\n", buf, count);
	...
	if(fw->size != touch_dev->cap_info.chip_fw_size)
	{
		...
		printk("TSP!!!!!!!!! fw->size : %d\n", fw->size);
		goto fail_upgrade_fw;
	}

	
	down(&touch_dev->work_proceedure_lock);	
	touch_dev->work_proceedure = TS_IN_UPGRADE;	
...
*/
static ssize_t zinitix_update_fw(struct device *dev, struct device_attribute *attr, char *buf)
{
	zinitix_touch_dev* touch_dev = dev_get_drvdata(dev);
	int ret = 0;
	char *filename_ptr;
	const struct firmware *fw = NULL;
	char *buf2 = "TSPFirmware.bin";
	int count = 15;
	printk("zinitix update fw : name = %s, size = %d\r\n", buf2, count);
	filename_ptr = kzalloc(count*2, GFP_KERNEL);
	...
	ret = sscanf(buf2, "%s", filename_ptr);
	if (ret != 1) {
		dev_err(dev, "invalid parameter\n");
		goto fail_fw;
	}
...
static ssize_t check_binary_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int count = 0;
	count = sprintf(buf, "%d\n", check_binary);
	return count;
...
static ssize_t check_binary_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int check_data;
	sscanf( buf, "%d", &check_data );
	if(check_data == 2) {
		printk("TSP firmware is not detected\n");
		check_binary = 2;
	}else if(check_data == 1) {
		printk("TSP firmware is detected\n");
		check_binary = 1;
	}else {
		printk("TSP firmware check binary error\n");
		printk("TSP check binary buf data is %s\n", buf);
	}
...
static DEVICE_ATTR(zinitix_fw_manager, S_IRUGO|S_IWUSR | S_IWGRP, zinitix_show_info, NULL);
static DEVICE_ATTR(zinitix_fw_upgrade, S_IRUGO|S_IWUSR | S_IWGRP, zinitix_update_fw, NULL);
static DEVICE_ATTR(check_binary, S_IRUGO|S_IWUSR | S_IWGRP, check_binary_show, check_binary_store);

static struct attribute *zinitix_attrs[] = {
         &dev_attr_zinitix_fw_manager.attr,
         &dev_attr_zinitix_fw_upgrade.attr,
         &dev_attr_check_binary.attr,
 	 NULL
};
...
static ssize_t zinitix_get_test_raw_data(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	{
		buf[4*180] = (touch_dev->cur_data[touch_dev->cap_info.total_node_num+1]&0xff);			//x_lsb
		buf[4*180+1] = ((touch_dev->cur_data[touch_dev->cap_info.total_node_num+1]>>8)&0xff);		//x_msb
		buf[4*180+2] = (touch_dev->cur_data[touch_dev->cap_info.total_node_num+2]&0xff);			//y_lsb
		buf[4*180+3] = ((touch_dev->cur_data[touch_dev->cap_info.total_node_num+2]>>8)&0xff);		//y_msb
		/*
		...
		buf[23] = ((touch_dev->cur_data[MAX_TEST_RAW_DATA+1]>>8)&0xff);		//y_msb
		*/
	}
	else	
	{
	...
	for(i=0;i<180;i++) {
		buf[4*i+0] = (char)(touch_dev->ref_data[i]&0xff);		//lsb
		buf[4*i+1] = (char)((touch_dev->ref_data[i]>>8)&0xff); 	//msb
		buf[4*i+2] = (char)(((s16)(touch_dev->cur_data[i]-touch_dev->ref_data[i]))&0xff);		//delta lsb
		buf[4*i+3] = (char)((((s16)(touch_dev->cur_data[i]-touch_dev->ref_data[i]))>>8)&0xff); 	//delta msb	
	}
	...
	*/
	...
	return 180*4+4; //NOISE_TEST_THRESHOLD_SIZE; // 20 Byte
...
*/
static ssize_t zinitix_set_testmode(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	//sscanf(buf, "%c", &value);
	down(&touch_dev->raw_data_lock);
	touch_dev->raw_mode_flag = set_test_mode;
	...
	else // ׽Ʈ  Խ
	{
	    ...
	    if (ts_write_reg(touch_dev->client, ZINITIX_TOUCH_MODE, 51)!=I2C_SUCCESS)
		{
...
static DEVICE_ATTR(get_touch_test_raw_data, S_IRUGO|S_IWUSR | S_IWGRP, zinitix_get_test_raw_data, NULL);
//static DEVICE_ATTR(set_touch_testmode, S_IRUGO|S_IWUSR | S_IWGRP, NULL, zinitix_set_testmode);
static DEVICE_ATTR(set_touch_testmode, S_IRUGO|S_IWUSR | S_IWGRP, zinitix_set_testmode, NULL);
...
static int zinitix_touch_probe(struct i2c_client *client, const struct i2c_device_id *i2c_id)
{
	...
	if(!ts_init_touch(touch_dev)) {
		printk(KERN_ERR "zinitix probe : init failed \n");
	...
	if (touch_dev->cap_info.Orientation & TOUCH_XY_SWAP)
	{
		input_set_abs_params(touch_dev->input_dev, ABS_MT_POSITION_Y, touch_dev->cap_info.MinX, touch_dev->cap_info.MaxX+1, 0, 0);
		input_set_abs_params(touch_dev->input_dev, ABS_MT_POSITION_X, touch_dev->cap_info.MinY, touch_dev->cap_info.MaxY+1, 0, 0);
	}
	else
	{
		input_set_abs_params(touch_dev->input_dev, ABS_MT_POSITION_X, touch_dev->cap_info.MinX, touch_dev->cap_info.MaxX+1, 0, 0);
		input_set_abs_params(touch_dev->input_dev, ABS_MT_POSITION_Y, touch_dev->cap_info.MinY, touch_dev->cap_info.MaxY+1, 0, 0);
	}
...
err_detect_failed:
	if (get_hw_revision() > 0x07) {
		printk("TSP is not detected. Turn off external LDO\n");
		gpio_direction_output(OMAP_GPIO_AMP_SHDN, 0);
		gpio_free(OMAP_GPIO_AMP_SHDN);
	}
