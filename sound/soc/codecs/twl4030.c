	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if(ucontrol->value.integer.value[0]){
#if ( defined( CONFIG_MACH_OMAP_AALTO ) ) // jypark72, to avoid build error
...
#endif
   		twl4030_write(codec, 0x3e,0x20);
#if ( defined( CONFIG_MACH_OMAP_AALTO ) ) // jypark72, to avoid build error #AALTO
		max97000_set_force_out_mode(FM_RADIO, twl4030_fm_device);		
#endif
    }
